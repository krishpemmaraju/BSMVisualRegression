...
test.beforeEach("Open the Order Capture URL", async ({ page }) => {
    await page.goto("https://vb04.wolseleyuk.com/ic/builder/rt/wol-order-capture/live/webApps/wol-order-capture/vp/",{timeout:80000});
...
test("Order Capture - Full Page Screenshot For Figma", async ({ page }) => {
    
    // await page.getByRole("heading").filter({hasText:'Order Capture'}).waitFor({state:'visible',timeout:12000})
    // await expect(page).toHaveScreenshot(["OrderCapture", "OrderCaptureFullScreenshot.png"], { fullPage: true ,
    //     maxDiffPixels: 10000,  threshold: 0.2,timeout:9000 });
})
