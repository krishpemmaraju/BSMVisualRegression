...
test.beforeAll("Open the Order Capture URL", async () => {
  ...
  try { await page.goto("https://vb04.wolseleyuk.com/ic/builder/rt/wol-order-capture/live/webApps/wol-order-capture/vp/"); } catch (error) { console.log(error) }
  await expect(page.getByRole('button').filter({ hasText: 'SSO' })).toBeVisible({ timeout: 35000 })
...
test("Validate Product List slot", async () => {
  ...
  await expect(prodSearchInputSlot).toBeVisible({timeout : 5000})
...
test("Validate Customer PO section info Slot", async () => {
  ...
  const isRequiredByDateAvailable = page.locator('.oj-inputdatetime-input.oj-text-field-input')
  const isDatePickerAvailable = page.locator('span[title="Select Date Time."]');
  const isDateSelectionPopUpDisplayed = page.locator('.oj-datepicker-content');
  ...
  const isCancelBtnAvailableUnderCustPOSection = page.locator('#btnCancel').filter({ has: page.locator('button[aria-label="Cancel"]')})
  const isContinueBtnAvailableUnderCustPOSection = page.locator('#btnSave').filter({ has: page.locator('button[aria-label="Continue"]')})
  const closeBtn = page.locator("button[aria-label='Close']")
  ...
  await expect(isRequiredByDateAvailable).toBeVisible({timeout: 5000})
  await expect(isDatePickerAvailable).toBeVisible({timeout: 5000})
  await isDatePickerAvailable.click();
  await expect(isDateSelectionPopUpDisplayed).toBeVisible({timeout: 5000})
...
test("Validate Add product to basket layout and Validate Auto populate fields", async () => {
  ...
  const isSaveBtnAvailable = page.getByText('Save', { exact: true });
  const isSaveAndExitBtnAvailable = page.getByText('Save and Exit', { exact: true })
  await expect(page.locator(isMoreActionsAvailable)).toBeVisible({ timeout: 3000 });
  await page.locator(isMoreActionsAvailable).click();
  await expect(page.locator(isMoreActionsMenuAvailable)).toBeVisible({ timeout: 5000 })
  await expect(isSaveBtnAvailable).toBeVisible({ timeout: 3000 })
  await expect(isSaveAndExitBtnAvailable).toBeVisible({ timeout: 3000 })
  await page.locator(isMoreActionsAvailable).click();
...
test("Validate Order Dialog pop up with Print and Edit Options", async () => {
  ...
  const waitForClearAllDialogBox = page.locator("#clearAllConfirmation")


  await clickOnClearAllBtn.click();
  await expect(waitForClearAllDialogBox).toBeVisible({ timeout: 6000 })
  const isCancelBtnAvailableOnClearAllDialog = page.locator('#clearAllConfirmation_layer button[aria-label="Cancel"]')
  const isContinueBtnAvailableOnClearAllDialog = page.locator('#clearAllConfirmation_layer button[aria-label="Continue"]')
  await expect(isCancelBtnAvailableOnClearAllDialog).toBeVisible({ timeout: 6000 })
  await expect(isContinueBtnAvailableOnClearAllDialog).toBeVisible({ timeout: 6000 })
  await isContinueBtnAvailableOnClearAllDialog.click()
  //    await customerContentSlotSelection.click();
